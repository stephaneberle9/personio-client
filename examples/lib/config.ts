 ...
 *
 * Everything here is *constant for a given Personio account* (unlike per-run CLI
 * flags such as `--from` / `--type`) and is *not* a secret (credentials stay in
 * `.env`). Supply it as a JSON file via `--config ./personio.config.json` to
 * keep invocations short. Every value also falls back to a `PERSONIO_*`
 * environment variable, which is handy server-side where shipping a file is
 * awkward. Precedence: config file > environment > built-in default.
 */
...
export interface ExampleConfig {
  /** Custom Report id for the ReportSource (env: `PERSONIO_REPORT_ID`). */
  reportId?: string;
  /**
